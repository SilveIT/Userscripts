...
(function() {
    ...
    async function loadPromoProducts() {
        try {
            ...
            const response = await fetch(window.location.origin + '/my/reviews/promo');
    ...
    function createVerifyButton(orderDiv, pointsElement, orderUrl, originalPoints) {
        ...
        button.addEventListener('click', async function() {
            ...
            try {
                const response = await fetch(orderUrl + '&force_platform=desktop');
                const html = await response.text();
    ...
    function processOrderList() {
        ...
        orderListWidgets.forEach(orderListWidget => {
            ...
            orderDivs.forEach(orderDiv => {
                ...
                const links = orderDiv.querySelectorAll('a');

                if (links.length === 2) {
                    // If 2 links - desktop version
                    appendTarget = orderDiv.querySelector('div') || orderDiv;
                    appendTarget.appendChild(wrapper);
                } else if (links.length > 2) {
                    // If more - probably mobile version
                    const targetLink = links[2];
                    appendTarget = targetLink.parentElement?.parentElement?.parentElement;

                    if (appendTarget) {
                        appendTarget.after(wrapper);
                    } else {
